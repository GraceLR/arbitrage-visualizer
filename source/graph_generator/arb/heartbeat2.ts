...
export default class Heartbeat2 {
    ...
    copy_stuff = () => {
        ...
        for (const exchange_pair of this.uniswap_pairs) {
            const c0 =
                crypto_dict[toLower(exchange_pair.crypto0.contract.address)];
            const c1 =
                crypto_dict[toLower(exchange_pair.crypto1.contract.address)];
    ...
    __execute_beat__ = async () => {
        try {
            ...
            const { uniswap_pairs, other_pairs, crypto_dict } =
                this.copy_stuff();
            ...
            if (other_pairs.length > 0) {
                const { blockNumber, results } = await run(call_list);
            ...
            const blockNumber =
                await UniswapV2ExchangeManager.refresh_liquidity(uniswap_pairs);
            ...
            // log_message(
            //     `beat failed, ${e.message.substring(0, 300)}`,
            //     LEVELS.ERROR
            // );
        }
    };
