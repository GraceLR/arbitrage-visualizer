...
export default class UniswapV2ExchangeManager extends ExchangeManager {
        ...
        const find_amounts = (
          ...
          for (const far_pair of far_pairs) {
            if (
              ...
              const r1: FixedNumber = exchange_pair.get_reserves(crypto);
              const r2: FixedNumber = far_pair.get_reserves(crypto);
              const amount = r2.isZero()
                ? FixedNumber.from(0)
                : r1.divUnsafe(r2).mulUnsafe(far_pair.reserves_in_usd);
              amounts.push(amount);
            }
          }
        };
  ...
  get_contracts_concurrent = (contract_addresses: string[], abi: string[]) => {
  ...
  generate_cryptos_and_exchange_pairs = async (crypto_dict: {
    ...
    if (R.toLower(Config[ConfigKeys.REGENERATE_GRAPHS]) === "true") {
      ...
      const [pruned_exchange_pairs, pruned_crypto_dict] =
        await UniswapV2ExchangeManager.prune_graph(exchange_pairs, crypto_dict);
      ...
      const vals = this.load_graph_from_file(this.exchanges().map((e) => e[0]));
    ...
    for (const pair of R.sort(
      (p1, p2) => (lessThan(p1.reserves_in_usd, p2.reserves_in_usd) ? 1 : -1),
      exchange_pairs
    )) {
      log_message(
